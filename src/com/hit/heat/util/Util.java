...
//		File file = new File(parentPath+"/"+path);
	...
	private static Location parseLocationFromLine(String line){
		...
		return new Location(Integer.parseInt(splits[0]), splits[1], splits[2], new Point(Integer.parseInt(splits[3]),Integer.parseInt(splits[4])));
	}
	
	/**
	 * 
	 * @des 将完整的Ip地址转换为缩略形式
	 * 		ff00:0:0:0:1:2:1:1 to ff00::1:2:1:1
	 * @param ip
	 * @return
	 */
	 ...
	 * @throws JSONException
	 */
	public static void writeSynConfigParamToFile( SynParameter synParameter,String path) throws IOException{
		BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(new File(path)));
...
//			JSONObject msgData = new JSONObject(json.getJSONObject("pama_data"));
...
//			System.out.println(new JSONObject(msgData.get("type")));
	 ...
	 * @param map 		不能为空
	 * @param selfId 	节点自身id
	 * @param parentId	节点父亲id（如果是br节点，为null即可）
	 */
...
//		FileInputStream fileInputStream = new FileInputStream(new File(parentPath+"/"+path));
		FileInputStream fileInputStream = new FileInputStream(new File(path));
		BufferedInputStream bufferedInputStream = new BufferedInputStream(fileInputStream);
		byte [] buffer = new byte[1024];
		int readLen;
	...
	public void sendInitHeart(boolean receive_frag,final boolean receiving_frag,final NetParameter parameter,final NettyClient remoteClient) {
	...
	public static byte[] getCorrectTimeMessage2(int flag, int h, int m, int s) {// 配置同步信息 content
		// 为bitmap
	...
	public static byte[] getCorrectTimeMessage(int h, int m, int s) {// 配置同步信息 content
		// 为bitmap
...
//		System.out.println();
	...
	public static long getLong(byte[] bb) {
		return (((long) (bb[0] & 0x00ff) << 40)
				| ((long) (bb[1] & 0x00ff) << 32)
				| ((long) (bb[2] & 0x00ff) << 24)
				| ((long) (bb[3] & 0x00ff) << 16)
				| ((long) (bb[4] & 0x00ff) << 8) | ((long) (bb[5] & 0x00ff) << 0));
	}
	...
	public static Energy Create_Energy(String selfId, byte[] others) {
		...
		System.arraycopy(message,
				GlobalDefines.GlobalCollectView.G_DEF_CRO_TIME_HOUR, node_time,
				0, length0);
		...
		System.arraycopy(message,
				GlobalDefines.GlobalCollectView.G_DEF_ENERGY_CPU, CPU_time, 0,
				length1);
		 //System.out.println(CPU_time[0]+"!");
		// LPM
		byte[] LPM_time = new byte[length1];
		System.arraycopy(message,
				GlobalDefines.GlobalCollectView.G_DEF_ENERGY_LPM, LPM_time, 0,
				length1);
		...
		System.arraycopy(message,
				GlobalDefines.GlobalCollectView.G_DEF_ENERGY_CYCLETIME, cycleTime, 0,
				2);
		//cycleTimeDirection
		...
		System.arraycopy(message,
				GlobalDefines.GlobalCollectView.G_DEF_ENERGY_SEND, Send_time,
				0, length1);
		// System.out.println(Send_time[0]);
		// Receiver time
		byte[] Receive_time = new byte[length1];
		System.arraycopy(message,
				GlobalDefines.GlobalCollectView.G_DEF_ENERGY_RECEIVE,
				Receive_time, 0, length1);
		// System.out.println(Receive_time[0]);

		// 電壓 2個字節
		byte[] Voltage = new byte[2];
		System.arraycopy(message,
				GlobalDefines.GlobalCollectView.G_DEF_ENERGY_VOLTAGE, Voltage,
				0, 2);
		// ParentID
		byte[] ParentID = new byte[2];
		System.arraycopy(message,
				GlobalDefines.GlobalCollectView.G_DEF_PARENTID_HIGH, ParentID,
				0, 2);
		//System.out.println(ParentID+"!");
		// beacon 2个 字节
		byte[] Beacon = new byte[2];
		System.arraycopy(message,
				GlobalDefines.GlobalCollectView.G_DEF_ENERGY_BEACON, Beacon, 0,
				2);
		// num_neighbors 2个 字节
		byte[] Num_neighbors = new byte[2];
		System.arraycopy(message,
				GlobalDefines.GlobalCollectView.G_DEF_ENERGY_NUM_NEGINBORS,
				Num_neighbors, 0, 2);
		// rtmetric 2 个 字节
		byte[] Rtmetric = new byte[2];
		System.arraycopy(message,
				GlobalDefines.GlobalCollectView.G_DEF_ENERGY_RTMERTIC,
				Rtmetric, 0, 2);
...
//				currenttime, 0, 1);
		...
		Energy ene = new Energy(NodeID, getLong(CPU_time), getLong(LPM_time),
				getLong(Send_time), getLong(Receive_time),
				Util.getInt(Voltage), Util.getInt(ParentID), SynTime, "" + Util.getInt(Beacon),
				 Util.getInt(Num_neighbors),  Util.getInt(Rtmetric),
				ReBoot, 0,getInt(cycleTime),""+cycleTimeDirection,""+currenttime);
